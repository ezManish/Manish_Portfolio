...
export class Desktop extends Component {
    ...
    openApp = (objId) => {
        // if opening github, route to in-portfolio Chrome with GitHub profile
        if (objId === "github") {
            const targetUrl = "https://github.com/ezManish";
            try {
                localStorage.setItem("chrome-url", targetUrl);
                localStorage.setItem("chrome-display-url", targetUrl);
            } catch (e) {}
            if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent("chrome-navigate", { detail: targetUrl }));
            }
            this.openApp("chrome");
            return;
        }
