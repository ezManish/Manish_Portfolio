...
export class Trash extends Component {
    constructor() {
        ...
        this.trashItems = [
            ...
            {
                name: "assignment_draft_final_v3.zip",
    ...
    focusFile = (e) => {
        ...
        if (target && target.children && target.children.length >= 2) {
            target.children[0].classList.toggle("opacity-60");
            target.children[1].classList.toggle("bg-ub-orange");
        }
    }
    ...
    showTrashItems = () => {
        return (
            <div className="flex-grow p-4 flex flex-wrap items-start content-start justify-start gap-y-4 gap-x-3 overflow-y-auto windowMainScreen">
                {
                    this.trashItems.map((item, index) => {
                        return (
                            <div
                                ...
                                className="flex flex-col items-center text-xs outline-none w-24 p-1.5 rounded hover:bg-white hover:bg-opacity-5 cursor-default group"
                                title={item.name}
                            >
                                <div className="w-14 h-14 flex items-center justify-center mb-1">
                                    <img className="w-12 h-12 object-contain" src={item.icon} alt="Ubuntu File Icons" />
                                </div>
                                <span className="text-center rounded px-1 w-full break-all leading-snug">
                                    {item.name}
                                </span>
                            </div>
                        )
